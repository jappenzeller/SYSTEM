...
%
% Save this file as: spherical_harmonics_viz.m
%
% Usage Examples:
%   spherical_harmonics_viz.static(3, 2, 300, 100)
%   spherical_harmonics_viz.animate(3, 2, 300, 30)
%   spherical_harmonics_viz.quantum([2, 3], [0, 1], [0.7, 0.3*exp(1i*pi/4)], 300, 30)

classdef spherical_harmonics_viz
    methods(Static)
        %% Main Visualization Functions
        
        function static(l, m, radius, resolution)
            ...
            Y_lm = spherical_harmonics_viz.computeSphericalHarmonic(l, m, THETA, PHI);
        ...
        function animate(l, m, worldRadius, fps)
            ...
            playBtn = uicontrol(controlPanel, 'Style', 'pushbutton', ...
                               ...
                               'Callback', @(src, evt) spherical_harmonics_viz.playCallbackSingle(fig, src));
            
            pauseBtn = uicontrol(controlPanel, 'Style', 'pushbutton', ...
                                'String', '❚❚ Pause', ...
                                'Units', 'normalized', ...
                                'Position', [0.11, 0.2, 0.08, 0.6], ...
                                'Callback', @(src, evt) spherical_harmonics_viz.pauseCallbackSingle(fig), ...
                                'Enable', 'off');
            ...
            restartBtn = uicontrol(controlPanel, 'Style', 'pushbutton', ...
                                  ...
                                  'Callback', @(src, evt) spherical_harmonics_viz.restartHarmonic(fig));
            ...
            modeBtn = uicontrol(controlPanel, 'Style', 'togglebutton', ...
                               ...
                               'Callback', @(src, evt) spherical_harmonics_viz.modeCallback(fig, src));
            ...
            phaseSlider = uicontrol(controlPanel, 'Style', 'slider', ...
                                   ...
                                   'Callback', @(src, evt) spherical_harmonics_viz.phaseCallback(fig, src));
            ...
            animTimer = timer('ExecutionMode', 'fixedRate', ...
                             'Period', 1/fps, ...
                             'TimerFcn', @(src, evt) spherical_harmonics_viz.updateHarmonic(fig));
            
            animData.timer = animTimer;
            fig.UserData = animData;
            
            % Initial draw
            spherical_harmonics_viz.updateHarmonic(fig);
            
            % Cleanup
            fig.CloseRequestFcn = @(src, evt) spherical_harmonics_viz.closeAnimationSingle(src, animTimer);
        end
        
        function quantum(l_values, m_values, coefficients, worldRadius, fps)
            ...
            playBtn = uicontrol('Style', 'pushbutton', ...
                                ...
                                'Callback', @(src, evt) spherical_harmonics_viz.playCallback(fig, src));
            
            pauseBtn = uicontrol('Style', 'pushbutton', ...
                                 'String', 'Pause', ...
                                 'Position', [90, 20, 60, 30], ...
                                 'Callback', @(src, evt) spherical_harmonics_viz.pauseCallback(fig), ...
                                 'Enable', 'off');
            
            restartBtn = uicontrol('Style', 'pushbutton', ...
                                   'String', 'Restart', ...
                                   'Position', [160, 20, 60, 30], ...
                                   'Callback', @(src, evt) spherical_harmonics_viz.restartCallback(fig));
            
            % Speed control slider
            speedSlider = uicontrol('Style', 'slider', ...
                                   'Min', 0.1, 'Max', 3, 'Value', 1, ...
                                   'Position', [250, 20, 150, 30], ...
                                   'Callback', @(src, evt) spherical_harmonics_viz.speedCallback(fig, src));
            ...
            animTimer = timer('ExecutionMode', 'fixedRate', ...
                             'Period', 1/fps, ...
                             'TimerFcn', @(src, evt) spherical_harmonics_viz.updateAnimation(fig), ...
                             'StopFcn', @(src, evt) spherical_harmonics_viz.stopAnimation(fig));
            
            animData.timer = animTimer;
            fig.UserData = animData;
            
            % Initial draw
            spherical_harmonics_viz.updateAnimation(fig);
            
            % Clean up timer when figure closes
            fig.CloseRequestFcn = @(src, evt) spherical_harmonics_viz.closeAnimation(src, animTimer);
        end
        
        function [vertices, colors] = mesh(l, m, latBands, lonSegments, worldRadius)
        ...
        function updateHarmonic(fig)
            ...
            switch data.displayMode
                case 'real'
                    ...
                    cmap = spherical_harmonics_viz.redblue(256);
        ...
        function updateAnimation(fig)
            ...
            for k = 1:length(data.l_values)
                Y_lm = spherical_harmonics_viz.computeSphericalHarmonic(data.l_values(k), data.m_values(k), ...
                                               data.THETA, data.PHI);
                omega = spherical_harmonics_viz.getEnergy(data.l_values(k), data.m_values(k));
            ...
            camlight;
            lighting gouraud;
            view(data.ax, 45, 30);  % Fixed camera angle
...
%% Example Usage (run these commands in MATLAB console after saving the file)
%
...
% spherical_harmonics_viz.static(2, 1, WORLD_RADIUS, 100);
%
% % Interactive single harmonic
% spherical_harmonics_viz.animate(3, 2, WORLD_RADIUS, 30);
%
% % Generate mesh data like Unity
% [verts, cols] = spherical_harmonics_viz.mesh(3, 2, LAT_BANDS, LON_SEGMENTS, WORLD_RADIUS);
%
% % Animate quantum superposition with controls
% spherical_harmonics_viz.quantum([2, 3], [0, 1], [0.7, 0.3*exp(1i*pi/4)], WORLD_RADIUS, 30);
%
% % Create interference pattern
% spherical_harmonics_viz.quantum([3, 3], [1, 3], [1, 0.8*exp(1i*pi/3)], WORLD_RADIUS, 30);